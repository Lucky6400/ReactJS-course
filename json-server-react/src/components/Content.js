...
const Content = ({items, setItems, theme}) => {
    ...
    */
    const handleCheck = (id) => {
        const listItems = items.map((item) => (item.id === id ? { ...item, done: !item.done } : item));
        setItems(listItems);
        ...
        <main className=" pt-3 container" >
            <ul style={{height: "40vh", overflowY: "scroll", scrollbarWidth:"thin"}}>
                {items.slice(0).reverse().map((item) => (
                    <li className={`list-none rounded-2 p-3 text-light my-2 col-lg-12 d-flex justify-content-between align-items-center`} style={{background: "rgba(055, 055, 055, 0.765)"}} key={item.id}>
                        <input type="checkbox"
                            onChange={() => handleCheck(item.id)}
                            className="" checked={item.done} name="" id="" />
